...
class Field(LayoutObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attrs.update(dict([(k.replace('_', '-'), conditional_escape(v)) for k, v in kwargs.items() if isinstance(v, str) or isinstance(v, bytes)]))
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, extra_context=None, **kwargs):
        ...
        if (self.form_class != None and (isinstance(form, MultiModelForm) or isinstance(form, MultiForm))):
            ...
            try:
                formToRender = self.form_class(instance=form.instance[self.form_class._meta.model.__name__.lower()])
            except:
